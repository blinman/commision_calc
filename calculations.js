...
function currentWeeklyCashOutTotal(singleTrans) {
  const transDate = singleTrans.date; // setting local variables for ease of use
  const userID = singleTrans.user_id;
  const cashOutAmount = singleTrans.operation.amount;
  let transYear = moment(transDate).format('YYYY'); // getting the year the transaction is taking place
  const transWeek = moment(transDate).format('W'); // getting the weel the transaction is taking place
  if ((transWeek == 53 || transWeek == 52) && (moment(transDate).format('MM') == '01')) { // check if the week is the last week of the previous year
    transYear = parseInt(transYear, 10) - 1; // change the year for storage to reflect the week it belongs to
  }
  ...
  const currentTotal = userCashOuts[userID][transYear][transWeek]; // set the currentTotal to whatever value the total was when the transaction was taking place
...
function feeForCashOutNatural(singleTrans, cashOutSettingsNatural) {
  const currentWeeklyTotal = currentWeeklyCashOutTotal(singleTrans); // retrieves the current weekly cash out total for that particular user
  const applicableAmount = weeklyApplicableAmount(singleTrans, currentWeeklyTotal, cashOutSettingsNatural); // check what amount the fees are applicable for
  const calculatedFee = percentageCalc(applicableAmount, cashOutSettingsNatural.percents); // calculates fee based on given percentage
